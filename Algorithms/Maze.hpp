...
struct Maze {
private:
    // whether to generate or read from file; or it could be hard-coded
    bool generate {true};
    // template: if $!generate
    // filename ? GUI itnerface: Browse
    FILE* file; // also, define the file format (maybe even choose a file extension to indicate it) <e.g.>.maze
    // metadata
    uint16_t width, height;
    // "line based" maze vs "adjacency matrix based" maze
    bool adjacency_matrix {true};
    // template: if $adJacency_matrix
    /*bool*/ uint8_t * grid;    // inside-object / outside-object
...
public:
    explicit Maze(uint16_t width_arg, uint16_t height_arg) : width(width_arg), height(height_arg) {
        engine.seed(seed);

        grid = new uint8_t[width_arg * height_arg];
    ...
    void verbose() {
        std::cout << "generate: " << ((generate)? "true" : "false") << "\nfile: " << file << "\nwidth: " << width
            << "\nheight: " << height << "\adjacency_matrix: " << ((adjacency_matrix)? "true" : "false")
            << "\nseed: " << seed << "\n";
    }
    ...
    void check_available_neighbours(int i, int j, Neighbours & neigh) {
        if((i - 1) > -1 && grid[(i - 1) * width + j] == false)
            neigh.up = true;
        if((j - 1) > -1 && grid[i * width + j - 1] == false)
            neigh.left = true;
    }
    void mark_left(int i, int j) {
        grid[i * width + j - 1] = true;
    }
    void mark_up(int i, int j) {
        grid[(i - 1) * width + j] = true;
    }
    void mark_random_available_neighbour(int i, int j, Neighbours & neigh) {
        uint64_t random = engine();

        if (neigh.left == false && neigh.up == true) {
            mark_up(i, j);
        }
        else if (neigh.left == false && neigh.left == true) {
            mark_left(i, j);
        }
        else if (neigh.up == true && neigh.left == true){
            random = engine() % 2;
            if(random == 0) { mark_up(i, j); }
            else { mark_left(i, j); }
        }
        else {}
    }
    void generate_BST_CA_random() {
        reset();
        Neighbours neigh;

        // iterator through the "2-2 lattice"
        for(int i = 0; i < height; i += 2) {
            for(int j = 0; j < width; j += 2) {
                grid[i * width + j] = true;

                check_available_neighbours(i, j, neigh);
                mark_random_available_neighbour(i, j, neigh);
            }
        }
        // for fun: "recursive chaining" the pseudo-random generator; if it were input-dependent, then "input poisoning" could be done
        seed = engine();
        engine.seed(seed);
    }
